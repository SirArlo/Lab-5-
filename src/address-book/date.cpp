// date.cpp
// Implementation of the Date class

#include "date.h"

Date::Date()
{
	_day = _default._day;
	_month = _default._month;
	_year = _default._year;
}

Date::Date (const int& TheDay, const Month& TheMonth, const int& TheYear): 
	_day{TheDay},
	_month{TheMonth},
	_year{TheYear}
{
	//do checks and stuff here
	if ( (_day <= 0) || (_year <= 0) )
	{
		throw InvalidDate::NegativeDate;
	}

	if (_day > daysInMonth())
	{
		throw InvalidDate::ExcessiveDays;
	}
}


int Date::day() const
{
	return _day;
}

Month Date::month() const
{
    return _month;
}

int Date::year() const
{
	return _year;
}

bool Date::isLeapYear() const
{
    // leap years must be divisible by 4 but not by 100
    // or divisible by 400
    if(((_year % 4 == 0) && (_year % 100 != 0)) || (_year % 400 == 0))
        return true;
    else
        return false;
}

int Date::daysInMonth() const
{
	switch (_month) {
		case Month::January: 
		case Month::March: 
		case Month::May: 
		case Month::July:
		case Month::August: 
		case Month::October: 
		case Month::December:
			return 31;	
		case Month::April: 
		case Month::June: 
		case Month::September:
		case Month::November:
			return 30;	
		case Month::February:
			if ( isLeapYear() ) return 29;
			else return 28;
		default:
			// should never reach here 
			assert(false); // See: http://www.cplusplus.com/reference/cassert/assert/
            return -1;
	}    
}

// Note, this standalone function is not part of the Date class
void printDate(const Date& date)
{
	cout << date.day() << "/"
		// cast to an integer to allow the month to be sent to the stream
        << static_cast<int>(date.month()) << "/"
		<< date.year()
		<< endl;
}

bool Date::operator==(const Date& rhs) const
{
// compare data members here
// return true for identical dates, false otherwise
	if ( (_day == rhs._day) && (_month == rhs._month) && (_year == rhs._year) )
		return true;
	else
		return false;
}

void Date::addDay()
{
	_day++;
	
	if ( (_day > daysInMonth()) && (_month == Month::December) )
	{
		_day = 1;
		_month = Month::January;
		_year++;
	}
	else
	{
		if (_day > daysInMonth())
		{
			//int monthValue = (Month)
			_day = 1;
			_month = nextMonth();
		}
	}

	if ( (_day <= 0) || (_year <= 0) )
	{
		throw InvalidDate::NegativeDate;
	}

	if (_day > daysInMonth())
	{
		throw InvalidDate::ExcessiveDays;
	}

	return;
}

Month Date::nextMonth()
{
	int monthValue = static_cast<int>(_month);
	monthValue++;
	Month e = static_cast<Month>(monthValue);
	return e;
}

Date Date::_default{1, Month::January, 1900};

void Date::setDefaultDate(int day, Month month, int year)
{
	_default._day = day;
	_default._month = month;
	_default._year = year;
	return;
}
